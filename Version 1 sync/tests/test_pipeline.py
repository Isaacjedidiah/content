...
def test_extract_populates_all_structural_fields():
    ...
    res = Extractor(client=client).extract([_element("has the primary metric")])
...
def test_low_confidence_escalates_to_tier2():
    ...
    res = Extractor(client=client).extract([_element("trigger coverage")])
...
def test_malformed_row_is_quarantined_not_fatal():
    ...
    res = Extractor(client=client).extract([_element("mix")])
    assert len(res.claims) == 1
...
class _RecordingClient:
    ...
    def extract(self, model_key, prompt, content, tool_schema, image_base64=None):
...
def test_hard_content_skips_tier1_pass():
    ...
    rc = _RecordingClient()
    Extractor(client=rc).extract([_el(modality=Modality.TABLE)])
    assert rc.calls == ["tier2"]


def test_easy_content_stays_tier1_when_confident():
    rc = _RecordingClient()
    Extractor(client=rc).extract([_el(modality=Modality.TEXT)])
...
def test_reactive_cascade_intact_for_low_confidence_text():
    ...
    lc = _LowConf()
    Extractor(client=lc).extract([_el(modality=Modality.TEXT)])
    assert lc.calls == ["tier1", "tier2"]


def test_routing_recorded_on_result():
    rc = _RecordingClient()
    res = Extractor(client=rc).extract([_el(modality=Modality.TABLE)])
...
def test_router_generates_sql_when_absent():
    ...
    out = router.answer("Show the primary metric for my firms", team_filter="capital")
...
def test_chart_figure_read_multimodally_and_tagged():
    rc = _RecMM()
    res = Extractor(client=rc).extract(
...
def test_figure_degrades_to_text_only_without_provider():
    rc = _RecMM()
    res = Extractor(client=rc).extract([_figure_el()])   # no provider
...
def test_text_provenance_parsed_figure_llm_estimated():
    ...
    assert Extractor(client=client).extract([text]).claims[0].citation_tier \
        == CitationTier.PARSED
...
class _CQClient:
    ...
    def complete(self, model, prompt, content):
        import json
...
def test_claim_link_resolves_above_threshold():
    link = match_claim("Primary metric strengthened", _cq_claims(), _CQClient(0, 0.9))
    assert link.matched_metric == "primary_ratio" and link.confidence == 0.9


def test_claim_link_unlinked_below_threshold():
    link = match_claim("Primary metric strengthened", _cq_claims(), _CQClient(0, 0.4))
    assert link.matched_metric is None       # not force-matched


def test_claim_link_null_not_forced():
    link = match_claim("vague statement", _cq_claims(), _CQClient(None, 0.0))
    assert link.matched_metric is None


def test_claim_link_malformed_json_safe():
    class _Bad:
        def complete(self, m, p, c):
            return _LLMResp(text="not json")
    link = match_claim("the primary metric rose", _cq_claims(), _Bad())
    assert link.matched_metric is None


def test_match_all_filters_non_claims():
    links = match_all(["Primary metric strengthened", "A heading", "coverage fell"],
...
def test_per_metric_review_thresholds():
...
def test_storage_slug_and_naming():
    ...
    assert sp.layer_table("gold", "fx", "exchange_report") == \
        "docextract.gold.fx_exchange_report"
...
def test_discovery_scans_team_report_layout(tmp_path):
    from docextract.custom.run_ingestion_job import discover_submissions
    import os
    os.makedirs(tmp_path / "fx" / "exchange_report")
    os.makedirs(tmp_path / "capital" / "adequacy")
    (tmp_path / "fx" / "exchange_report" /
     "FRN123_Acme_FX_2026-06-30.pdf").write_text("x")
    (tmp_path / "capital" / "adequacy" /
     "FRN999_Beta_Cap_2026-03-31.pdf").write_text("x")
    (tmp_path / "capital" / "adequacy" / ".hidden").write_text("x")
    subs = discover_submissions(str(tmp_path))
    assert len(subs) == 2                        # hidden skipped
    fx = [s for s in subs if s["team"] == "fx"][0]
    assert fx["report_type"] == "exchange_report"
    assert fx["entity_ref"] == "FRN123"            # from filename, not path
...
def test_first_turn_not_rewritten_and_recorded(tmp_path):
    ...
    try:
        ...
        r = _sess_router()
        a = r.answer("What is the financial risk of firm A",
...
def test_followup_reference_is_resolved(tmp_path):
    ...
    try:
        ...
        r = _sess_router()
        r.answer("What is the financial risk of firm A", team_filter="capital", session=sess)
        a2 = r.answer("Will firm A survive from the figure", team_filter="capital", session=sess)
...
def test_feedback_capture_and_validation(tmp_path):
    ...
    try:
        ...
        r = _sess_router()
        a = r.answer("What is the financial risk of firm A", team_filter="capital", session=sess)
...
def test_stateless_call_still_works():
    ...
    r = _sess_router()
    a = r.answer("What is the financial risk of firm A", team_filter="capital")
...
def test_supervisor_feedback_summary(tmp_path):
    ...
    try:
        ...
        r = _sess_router()
        a1 = r.answer("risk of firm A", team_filter="capital", session=sess)
        a2 = r.answer("risk of firm B", team_filter="capital", session=sess)
