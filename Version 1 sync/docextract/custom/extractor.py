...
class Extractor:
    ...
    def extract(self, elements: list[Element],
                document_bytes: Optional[bytes] = None,
                filename: Optional[str] = None,
                page_image_provider=None) -> ExtractionResult:
        """Extract claims from every element.
        ...
        for el in elements:
            ...
            self._extract_element(el, prompt, result, document_bytes,
                                  filename, page_image_provider)
        return result
    ...
    def _extract_element(self, el: Element, prompt: str,
                         result: ExtractionResult,
                         document_bytes: Optional[bytes] = None,
                         filename: Optional[str] = None,
        ...
        if decision.tier == "tier2":
            ...
            resp = self._client.extract("tier2", prompt, el.content,
            ...
            resp = self._client.extract("tier1", prompt, el.content,
                                        EXTRACTION_TOOL_SCHEMA, image_b64)
