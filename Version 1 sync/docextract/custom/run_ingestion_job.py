...
def discover_submissions(root: str) -> list[dict]:
    """Scan <root>/<team>/<report_type>/<file>. Every team folder, every
    report_type folder under it, every document within."""
    subs: list[dict] = []
    ...
    for team in sorted(os.listdir(root)):
        ...
        for report_type in sorted(os.listdir(team_dir)):
            ...
            for fname in sorted(os.listdir(rt_dir)):
                if fname.startswith(".") or not os.path.isfile(
                        os.path.join(rt_dir, fname)):
                    continue
                subs.append({
                    "path": os.path.join(rt_dir, fname),
                    "entity_ref": entity_ref_from_filename(fname),
                    "team": team,
...
def main() -> None:
    ...
    result = process_batch(subs, search_store=search_store,
