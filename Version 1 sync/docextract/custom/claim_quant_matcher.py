...
def _confirm(claim_text: str, candidates: list[_Candidate],
             client: LLMClient) -> tuple[Optional[str], float]:
    if not candidates:
        return None, 0.0
    block = "\n".join(
    ...
    resp = client.complete(CLASSIFIER_MODEL, _CONFIRM_PROMPT, content)
    try:
...
def match_claim(claim_text: str, claims: list[Claim],
                client: LLMClient) -> ClaimQuantLink:
    candidates = _retrieve_candidates(
        claim_text, claims, CONFIG.claim_quant.top_n_candidates)
    metric, conf = _confirm(claim_text, candidates, client)
...
def match_all(narrative_texts: list[str], claims: list[Claim],
              client: LLMClient) -> list[ClaimQuantLink]:
    return [match_claim(t, claims, client)
            for t in narrative_texts if looks_like_claim(t)]
